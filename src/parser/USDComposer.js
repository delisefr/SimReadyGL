...
class USDComposer {
	...
	applyTransform( obj, fields, attrs = {} ) {
			...
			for ( let i = 0; i < xformOpOrder.length; i ++ ) {
				...
				if ( opName === 'xformOp:transform' ) {
					...
					if ( q && q.length === 4 ) {
						...
						// USD orient is (real, i, j, k) = (w, x, y, z)
		...
		if ( data[ 'xformOp:rotateXYZ' ] ) {
			...
			const r = data[ 'xformOp:rotateXYZ' ];
			// USD rotateXYZ: matrix = Rx * Ry * Rz → Three.js Euler 'ZYX'
			obj.rotation.set(
				r[ 0 ] * Math.PI / 180,
				r[ 1 ] * Math.PI / 180,
				r[ 2 ] * Math.PI / 180,
		...
		if ( data[ 'xformOp:orient' ] ) {
			...
			if ( q.length === 4 ) {
				...
				// USD orient is (w, x, y, z), Three.js is (x, y, z, w)
	...
	_resolveFilePath( refPath ) {
		...
		if ( this.basePath ) {
			...
			cleanPath = this.basePath + '/' + cleanPath;

		}

		// Normalize .. segments (e.g., "a/b/c/../../d" → "a/d")
	...
	_getReference( spec ) {
		...
		if ( spec.fields.payload ) {
			...
			// PayloadListOp returns an array of payloads
		...
		for ( const path in this.specsByPath ) {
			...
			if ( orientSpec?.fields?.timeSamples ) {
				...
				for ( let i = 0; i < times.length; i ++ ) {
					...
					const q = values[ i ];
					// USD orient is (w, x, y, z), Three.js quaternion is (x, y, z, w)
