...
class USDCParser {
	...
	_readStrings() {
		...
		// STRINGS section format: uint64 count, then count × uint32 token indices.
		// Each uint32 is a token index: strings[i] → tokens[strings[i]] gives the string.
	...
	_readPaths() {
		...
		if ( this.version.major === 0 && this.version.minor < 4 ) {
			...
			// Format: numPaths (already read), numElements (uint64),
			// then 3 compressed arrays each with compressedSize (uint64) + data.
			// numElements is the number of entries in the compressed tree
			// (may be <= numPaths if some path slots are unused).
			const numElements = reader.readUint64();
		...
		switch ( type ) {
			...
			case TypeEnum.StringVector: {
				...
				for ( let i = 0; i < count; i ++ ) {
					...
					const strIdx = reader.readUint32();
					result.push( this.tokens[ this.strings[ strIdx ] ] );
			...
			case TypeEnum.ReferenceListOp: {

				// ReferenceListOp: same list-op structure as PathListOp
				// but each item is a Reference (assetPath + primPath + layerOffset + customData)
				...
				const readRefList = () => {
					...
					for ( let i = 0; i < itemCount; i ++ ) {
						...
						const layerOffset = reader.readFloat64();
						const layerScale = reader.readFloat64();

						// Read customData dictionary (inline: count + key/value pairs)
						const dictCount = reader.readUint64();
						for ( let j = 0; j < dictCount; j ++ ) {

							reader.readUint32(); // key token index
							reader.readUint32(); // value rep lo
							reader.readUint32(); // value rep hi

						}

						const assetPath = this.tokens[ this.strings[ strIdx ] ] || '';
						refs.push( {
							assetPath: assetPath,
							primPath: this.paths[ pathIdx ] || '',
							layerOffset: { offset: layerOffset, scale: layerScale }
						} );
				...
				let explicitRefs = null, addRefs = null, prependRefs = null, appendRefs = null;
				if ( hasExplicitItems ) explicitRefs = readRefList();
				if ( hasAddItems ) addRefs = readRefList();
				if ( hasPrependItems ) prependRefs = readRefList();
				if ( hasAppendItems ) appendRefs = readRefList();
				if ( hasDeleteItems ) readRefList();
				if ( hasReorderItems ) readRefList();

				if ( prependRefs && prependRefs.length > 0 ) return prependRefs;
				if ( explicitRefs && explicitRefs.length > 0 ) return explicitRefs;
				if ( appendRefs && appendRefs.length > 0 ) return appendRefs;
				if ( addRefs && addRefs.length > 0 ) return addRefs;

				return null;

			}

			case TypeEnum.PayloadListOp: {

				// PayloadListOp: same list-op structure as PathListOp
				// but each item is a Payload (assetPath + primPath + layerOffset)
				const flags = reader.readUint8();
				const hasExplicitItems = ( flags & 0x02 ) !== 0;
				const hasAddItems = ( flags & 0x04 ) !== 0;
				const hasDeleteItems = ( flags & 0x08 ) !== 0;
				...
				let explicitPayloads = null, addPayloads = null, prependPayloads = null, appendPayloads = null;
				if ( hasExplicitItems ) explicitPayloads = readPayloadList();
				if ( hasAddItems ) addPayloads = readPayloadList();
				if ( hasPrependItems ) prependPayloads = readPayloadList();
				if ( hasAppendItems ) appendPayloads = readPayloadList();
				if ( hasDeleteItems ) readPayloadList();
				if ( hasReorderItems ) readPayloadList();

				if ( prependPayloads && prependPayloads.length > 0 ) return prependPayloads;
				if ( explicitPayloads && explicitPayloads.length > 0 ) return explicitPayloads;
				if ( appendPayloads && appendPayloads.length > 0 ) return appendPayloads;
				if ( addPayloads && addPayloads.length > 0 ) return addPayloads;
